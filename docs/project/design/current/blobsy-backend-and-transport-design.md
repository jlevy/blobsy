...
| `command` | Validates at least one command configured; verifies command binary exists in PATH via `which` | Commands configured, binary available |
...
Command backends now perform basic health checks:

1. **Configuration check** — Verifies at least one of `push_command` or `pull_command`
   is configured.
2. **Binary availability** — Extracts the first token (binary name) from the configured
   command template and verifies it exists in `PATH` using `which`.

This catches common setup issues (missing binaries, typos) before attempting transfers.
`blobsy doctor` also checks each configured command binary independently.

**Testing Procedure:**

For more thorough testing beyond the automatic health check, manually test push/pull:
