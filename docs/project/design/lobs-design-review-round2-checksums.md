# Review: lobs Checksums and Integrity Model (Round 2)
...
`x-amz-checksum-sha256`) already cover transfer integrity, and that lobs-level hashing
...
Lobs does not need to re-solve transfer integrity.
...
**Verdict: No lobs-level work needed.
...
But **branch switching is a core lobs workflow**, not an edge case.
...
- `lobs status` shows every tracked file as “changed” (false positives)
- `lobs push` re-uploads everything unnecessarily
...
On `lobs push` for a directory, lobs already reads every file to upload it.
...
For lobs, where branch switching is a core workflow and mtime is guaranteed to be
...
With per-file hashes in the manifest, `lobs verify` for directories becomes trivial:
...
Post-download verification (checking files after `lobs pull`) is also a nice-to-have.
...
**Verdict: Implement `lobs verify` for directories once manifest hashes exist.
...
**`lobs status` (fast, offline):** Compare local file sizes against manifest.
...
For definitive verification: `lobs verify`.

**`lobs push` (accurate, pre-upload):** Hash each local file.
...
**`lobs pull` (with manifest):** Fetch manifest.
...
- **Single files:** SHA-256 in the pointer file.
  Enables change detection, `lobs verify`, and meaningful `git diff` on pointers.
...
- `sha256` should be the only default.
  MD5 is weaker and not meaningfully faster for the file sizes lobs targets.
...
| `lobs status` | Size comparison only (fast, offline, no file reads). |
| `lobs push` change detection | Hash comparison against manifest. The key payoff: prevents unnecessary re-uploads after branch switches. |
| `lobs pull` change detection | Hash + size comparison against manifest. Download only what differs. |
| `lobs verify` for directories | Implement using manifest hashes. Not a V1 launch blocker but easy once hashes exist. |
...
| Transfer integrity | **No lobs-level work.** Transport tools provide adequate coverage. |

### Reassessment of Round 1 Issues

| Round 1 ID | Round 1 severity | Revised severity | Rationale |
| --- | --- | --- | --- |
| C1 | Critical | **Critical (confirmed), reframed** | Not about transfer integrity (transport handles that). Critical because without manifest hashes, `lobs push` after `git checkout` re-uploads everything. The core branch-switching workflow is broken without hashes. |
